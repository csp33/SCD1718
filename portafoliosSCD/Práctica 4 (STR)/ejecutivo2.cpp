...
void TareaD() { Tarea( "D", milliseconds( 200) );  }
void TareaE() { Tarea( "E", milliseconds( 240) );  }
...
/****************************************** /
   ...
   -> ¿ cual es el mínimo tiempo de espera que queda al final de las
      iteraciones del ciclo secundario con tu solución ?
       ...
       El mínimo tiempo de espera es de 10ms (en el segundo ciclo, cuando se ejecuta D)

   -> ¿ sería planificable si la tarea D tuviese un tiempo cómputo de
        250 ms ?

        Sí, es más, no sobraría tiempo de CPU en el Ts en el que se ejecuta D, ya que al ejecutarse A, B y D se completaría el período
        de 500ms (100+150+250)
