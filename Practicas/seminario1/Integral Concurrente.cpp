...
{
  cout<<endl<<endl<<"Calculando integral secuencial..."<<endl<<endl;
...
{
  double suma=0.0;
  long ultimo_valor=(indice+1)*m/n;   //Cada hebra hace la suma parcial de m/n puntos del dominio.
  for(long i=indice*m/n;i<ultimo_valor;i++)
...
{
  cout<<endl<<endl<<"Calculando integral concurrente..."<<endl<<endl;
  future<double> suma_parcial[n]; //Creamos el vector que iniciará las hebras y almacenará las sumas parciales
  for(long i=0;i<n;i++){
    suma_parcial[i]=async(launch::async,funcion_hebra,i); //Lanzamos la hebra i
  }

  double suma_total=0.0;

  for(long i=0;i<n;i++)
    suma_total+=suma_parcial[i].get();    //Realizamos la suma total y dividimos entre el número de muestras para hallar el promedio.
  suma_total/=m;
  return suma_total;
}
// -----------------------------------------------------------------------------
