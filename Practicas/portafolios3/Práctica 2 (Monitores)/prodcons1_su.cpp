...
class ProdCons1SU : public HoareMonitor
{
 ...
 CondVar puedo_producir;
 CondVar puedo_consumir;
...
{
   primera_libre = 0 ;
   puedo_producir=newCondVar();
   puedo_consumir=newCondVar();
}
// -----------------------------------------------------------------------------
...
int ProdCons1SU::leer(  )
{
   ...
   if ( primera_libre == 0 )	//Si no se ha producido
      puedo_consumir.wait();
