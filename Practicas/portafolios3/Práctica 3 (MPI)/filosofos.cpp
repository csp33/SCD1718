...
void funcion_filosofos( int id )
{
  ...
  while ( true )
  {
    if (id == 0) {   //Si es el primero, coge los tenedores al revés para no bloquear.
      cout << "Filósofo " << id << " solicita ten. der." << id_ten_der << endl;
      MPI_Ssend(&peticion, 1, MPI_INT, id_ten_der, etiq_COGER, MPI_COMM_WORLD); // ... solicitar tenedor derecho (completar)

      cout << "Filósofo " << id << " solicita ten. izq." << id_ten_izq << endl;
      MPI_Ssend(&peticion, 1, MPI_INT, id_ten_izq, etiq_COGER, MPI_COMM_WORLD); // ... solicitar tenedor izquierdo (completar)s
    }
    else {
      cout << "Filósofo " << id << " solicita ten. izq." << id_ten_izq << endl;
